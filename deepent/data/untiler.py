        ...
        for tile in tiles:
            ...
            if predictions.has("pred_masks"):
                for (polygon, area, cls) in _format_predictions(predictions, height, width):
                    shp.poly(shift_polygon(polygon, x_scale, y_scale, x_shift, y_shift))
                    # TODO: is class the id number or the string????????
                    shp.record(self._tree_id, area * x_scale * y_scale, cls)
                    self._tree_id += 1


def shift_polygon(polygon, x_scale, y_scale, x_shift, y_shift):
    """
    ...
    # TODO: How to deal with nested polygons(i.e. holes)?
